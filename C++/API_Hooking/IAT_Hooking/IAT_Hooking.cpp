...
int main()
{
	...
	LPVOID imageBase = GetModuleHandleA(NULL); // Get a handle on itself
	PIMAGE_DOS_HEADER dosHeaders = (PIMAGE_DOS_HEADER)imageBase; // Get the DOS headers
	PIMAGE_NT_HEADERS ntHeaders = (PIMAGE_NT_HEADERS)((DWORD_PTR)imageBase + dosHeaders->e_lfanew); // Get Optional headers

	PIMAGE_IMPORT_DESCRIPTOR importDescriptor = NULL;
	IMAGE_DATA_DIRECTORY importsDirectory = ntHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT]; // IAT entry point ? 
	importDescriptor = (PIMAGE_IMPORT_DESCRIPTOR)(importsDirectory.VirtualAddress + (DWORD_PTR)imageBase); // RVA ?
	...
	while (importDescriptor->Name != NULL)
	{
		...
		if (library)
		{
			...
			while (originalFirstThunk->u1.AddressOfData != NULL)
			{
				...
				if (strncmp(functionName->Name, "MessageBoxA", 0) == 0) // std::string(functionName->Name).compare("MessageBoxA") == 0)
				{
